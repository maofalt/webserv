...
int Server::updateEpoll(int epoll_fd, int clientFd, std::vector<t_epollSwitch>& epollSwitch) {
    ...
    while (it != epollSwitch.end()) {
		...
		log_message(Logger::DEBUG, "epollSwitch fd: %d, mode: %d", it->fd, it->mode);
        
		int op = determineOperation(it, clientFd);
		if (op == -1) {
			log_message(Logger::DEBUG, "fd %d is going to be deleted BUT fd i not tracked", it->fd);
		}
        
		int mode = determineMode(it);

        if (changeClientEpollMode(epoll_fd, it->fd, clientFd, mode, op)) {
...
int Server::determineOperation(std::vector<t_epollSwitch>::iterator it, int clientFd) {
    ...
    if (trackFds.find(it->fd) != trackFds.end()) {
		...
		log_message(Logger::DEBUG, "fd %d is going to be added", it->fd);
        op = EPOLL_CTL_ADD;
        trackFds.insert(it->fd);
        if (it->fd != clientFd) {
			log_message(Logger::DEBUG, "fd %d is a cgiFd, ClientHandler PARENT %d", it->fd, clientFd);
...
int		Server::changeClientEpollMode(int epoll_fd, int eventFD, int clientFd, u_int32_t mode, int op) {

	struct epoll_event ev;
	if (mode) {
		ev.events = mode | EPOLLET;
		ev.data.fd = eventFD;
		if (epoll_ctl(epoll_fd, op, eventFD, &ev) == -1) {
		log_message(Logger::ERROR, "epoll_ctl: problem with during switch");
		return -1;
		}
	} else
	...
	if (op == EPOLL_CTL_DEL ) {
		    if (isCgiActiveForClient(clientFd)) {
            // If CGI is still active, adjust the timeout and postpone the deletion
            log_message(Logger::DEBUG, "CGI is still active for fd %d, adjusting timeout and postponing deletion", eventFD);
            //adjustTimeoutForClient(eventFD); // Implement this function to adjust the timeout
            return 0;
        }
		
		trackFds.erase(eventFD);
		if (eventFD != clientFd) //itmeans it is a cgi
			_cgiFdsToClientFd.erase(eventFD);
		else
			clientHandlers.erase(eventFD);
		removeTimeoutEvent(eventFD);
		
		//check with stat fi fd is openif yes closeis
		log_message(Logger::DEBUG, "fd %d is going to be deleted", eventFD);
		struct stat buf;
		if (fstat(eventFD, &buf) == 0)
			close(eventFD);
	}
	return 0;
}

int Server::isCgiActiveForClient(int clientFd) {
    std::map<int, ClientHandler>::iterator it = clientHandlers.find(clientFd);
    if (it != clientHandlers.end()) {
        ClientHandler& clientHandler = it->second;
        return (clientHandler.getFdCgiInOpened() || clientHandler.getFdCgiOutOpened());
    }
    return 0;
}
