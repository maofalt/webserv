...
/*   Updated: 2023/10/10 15:42:06 by rgarrigo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool digitsOnly(const std::string& str) {
...
bool ConfigValidator::validateConfigData(std::map<std::string, std::vector<std::string> >& confData, const std::string& contextType) {
    for (std::map<std::string, std::vector<std::string> >::iterator it = confData.begin(); it != confData.end(); ++it) {
        //if it->first has only nbrs, create full context with "errorPages" rather than  it->first
        std::string tmpStr;
        if (digitsOnly(it->first))
            tmpStr = contextType + "." + "errorPages";
        else
            tmpStr = contextType + "." + it->first;
        const std::string fullContext = tmpStr;
    ...
    for (std::set<std::string>::const_iterator it = _mandatorySections.begin(); it != _mandatorySections.end(); ++it) {
        if(it->find(contextType + ".") != std::string::npos) {
            ...
            if (confData.find(parameter) == confData.end()) {
                log_message(Logger::WARN, "Mandatory %s config key [%s] not found", contextType.c_str(), parameter.c_str());
